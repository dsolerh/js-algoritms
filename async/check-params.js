...
function checkFloat(value, fullCheck, path) {
...
exports.CheckParams = class CheckParams {
    ...
    if (values.constructor != Array) {
      ...
      for (var j = 0; j < values.length; j++) {
        ...
        for (var i = 0; i < struct.length; i++) {
          ...
          if (
            values[j][struct[i].name] == undefined &&
            struct[i].defaultValue != undefined
          ) {
          ...
          if (struct[i].required == true) {
            if (values[j][struct[i].name] != undefined) {
              self.checkParams(
                ...
                values[j],
                [struct[i]],
                path + "." + fullCheck.name + ".[" + j.toString() + "]"
              );
            } else {
              self.errors.push({
                field:
  ...
  checkParams(self, listToCheck, listOfExpectedParams, path = "") {
    ...
    for (var i = 0; i < listOfExpectedParams.length; i++) {
      check = false;
      if (
        listToCheck[listOfExpectedParams[i].name] != undefined &&
        listOfExpectedParams[i].dependsOf != undefined
      ) {
      ...
      if (
        listOfExpectedParams[i].defaultValue != undefined &&
        listToCheck[listOfExpectedParams[i].name] == undefined
      ) {
        /**
         * Si le tengo un valor por defecto y no viene ninguno, entonces pongo ese
         * */
        listToCheck[listOfExpectedParams[i].name] =
          listOfExpectedParams[i].defaultValue;
      }
      ...
      if (check) {
        ...
        if (self.checkSimpleTypes[listOfExpectedParams[i].type] != undefined) {
           ...
           * */
          self.checkSimpleTypes[listOfExpectedParams[i].type](
            self,
            listToCheck[listOfExpectedParams[i].name],
            listOfExpectedParams[i],
            path
          );
        } else if (
          self.checkComplexTypes[listOfExpectedParams[i].type] != undefined
        ) {
          /**
           * es un atributo complejo
           * */
          self.checkComplexTypes[listOfExpectedParams[i].type](
            self,
            listToCheck[listOfExpectedParams[i].name],
            listOfExpectedParams[i],
            path
